...
3. Require explicit provider config in inventory or a separate non-committed local config.
4. Keep destructive confirmation tokens separate from routine SSH tool inputs.
5. Return host IDs, provider instance IDs, exact actions taken, and which deploy key or image path was involved.
6. Regenerate indexes after provisioning so the topology remains current.
